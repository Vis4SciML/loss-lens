...
function render(
    ...
    const xScale = d3
        .scaleLinear()
        .domain(d3.extent(wholeNodes, (node) => node.x))
        .range([0, w])

    const yScale = d3
        .scaleLinear()
        .domain(d3.extent(wholeNodes, (node) => node.y))
        .range([0, h])

    const linkThickness = d3
        .scaleLinear()
        .domain(d3.extent(wholeLinks, (link) => link.weight))
        .range([2, 20])
    ...
    const modelColorMap = modelList.reduce((acc, model, index) => {
    ...
    const barScale = d3
        .scaleRadial()
        .domain([
            d3.min(nodes, (d) => d3.min(d.localFlatness) ?? 0),
            d3.max(nodes, (d) => d3.max(d.localFlatness) ?? 0),
        ])

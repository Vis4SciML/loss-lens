...
class CKA_PINN:
    def __init__(self,
        ...
        self.model1 = model1
        self.model2 = model2
        self.whole_model1 = model1
        self.whole_model2 = model2
        ...
        if len(list(model1.dnn.modules())) > 150 and model1_layers is None:
        ...
        if len(list(model2.dnn.modules())) > 150 and model2_layers is None:
        ...
        self._insert_hooks()
        self.model1 = self.model1.dnn.to(self.device)
        self.model2 = self.model2.dnn.to(self.device)

        # self.model1 becomes dnn
        self.model1.eval()
        self.model2.eval()
    ...
    def _insert_hooks(self):
        # Model 1
        for name, layer in self.model1.dnn.named_modules():
        ...
        # Model 2
        for name, layer in self.model2.dnn.named_modules():
    ...
    def compare(self,
        ...
        for x1 in tqdm(dataloader1, desc="| Comparing features |", total=num_batches):
            if same_loader:
                ...
                x2 = next(iter(dataloader2))
            self.model1_features = {}
            self.model2_features = {}
            _ = self.whole_model1.predict(x1.to(self.device))
            _ = self.whole_model2.predict(x2.to(self.device))
